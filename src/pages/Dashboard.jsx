  ...
  return (
    <div className='w-full md:w-2/3 bg-white px-2 md:px-4 pt-4 pb-4 shadow-md rounded'>
      <table className='w-full'>
        ...
        <tbody>
          {tasks?.map((task, id) => (
            <TableRow key={id} task={task} ICONS={ICONS} />
          ))}
        </tbody>
      </table>
    </div>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:8800/task/dashboard", {
          headers: {
  ...
  const totals = summary.tasks || {}; // Default to an empty object if tasks are not defined
